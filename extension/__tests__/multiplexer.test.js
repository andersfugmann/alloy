...
describe("multiplexer port connection", () => {
  test("sub-port connects and registers onMessage listener", () => {
  ...
  test("request through sub-port is forwarded to native port", async () => {
    ...
    triggerPortMessage(subPort, { id: 1, payload: { command: "status", params: null } });
    await new Promise((resolve) => setTimeout(resolve, 10));
    ...
    expect(forwarded).toHaveProperty("id");
  ...
  test("response from native port is forwarded back to sub-port", async () => {
    ...
    triggerPortMessage(subPort, { id: 42, payload: { command: "status", params: null } });
    await new Promise((resolve) => setTimeout(resolve, 10));
    ...
    triggerPortMessage(nativePort, {
      id: forwarded.id,
      payload: ["Success", statusPayload],
    });
    await new Promise((resolve) => setTimeout(resolve, 10));

    // The sub-port should receive a response with the original ID
    expect(subPort.postMessage).toHaveBeenCalled();
    const response = subPort.postMessage.mock.calls[0][0];
    expect(response.id).toBe(42);
    expect(response.payload[0]).toBe("Success");
  });
  ...
  test("broadcast push is forwarded to all connected sub-ports", async () => {
    ...
    triggerPortMessage(nativePort, {
      id: 0,
      payload: ["Navigate", { url: "https://pushed.example.com" }],
    });
    await new Promise((resolve) => setTimeout(resolve, 10));
  ...
  test("disconnected sub-port stops receiving broadcasts", async () => {
    ...
    triggerPortMessage(nativePort, {
      id: 0,
      payload: ["Navigate", { url: "https://example.com" }],
    });
    await new Promise((resolve) => setTimeout(resolve, 10));
  ...
  test("multiple sub-ports can proxy requests independently", async () => {
    ...
    triggerPortMessage(subPort1, { id: 1, payload: { command: "status", params: null } });
    triggerPortMessage(subPort2, { id: 1, payload: { command: "get_rules", params: null } });
    await new Promise((resolve) => setTimeout(resolve, 10));
    ...
    expect(fwd1.id).not.toBe(fwd2.id);

    // Respond to both (in reverse order to test ID routing)
    triggerPortMessage(nativePort, { id: fwd2.id, payload: ["Success", []] });
    triggerPortMessage(nativePort, { id: fwd1.id, payload: ["Success", { registered_tenants: [], uptime_seconds: 0 }] });
    await new Promise((resolve) => setTimeout(resolve, 10));

    // Each sub-port should get a response with its original ID
    expect(subPort1.postMessage).toHaveBeenCalled();
    expect(subPort2.postMessage).toHaveBeenCalled();
